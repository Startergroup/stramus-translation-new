    ...
    const { errors, form, handleSubmit } = getForm({
      items: [
        ...
        },
        {
          ...
          props: {
            isCodeVisible: false,
            label: computed(() => t('message.email')),
            placeholder: computed(() => t('message.enterEmail')),
            type: inputTypes.TEXT
          }
        }
      ],
      ...
      validationSchema: yup.object().shape({
        ...
        email: yup.string().email().required(computed(() => t('message.requiredField')))
      })
    })
    const onSubmit = handleSubmit(async (values) => {
      const { success, message = '' } = await store.dispatch('auth/guestLogin', values)
